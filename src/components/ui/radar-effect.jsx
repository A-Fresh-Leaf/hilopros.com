...
export const Radar = ({ className }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className="relative z-40 h-[1px] w-full bg-gradient-to-r from-transparent via-brand-teal/80 to-transparent" />
      </div>
      {/* Concentric circles */}
      {circles.map((_, idx) => (
        <Circle
          style={{
            height: `${(idx + 1) * 5}rem`,
            width: `${(idx + 1) * 5}rem`,
            border: `1px solid rgba(46, 158, 138, ${1 - (idx + 1) * 0.1})`,
          }}
          key={`circle-${idx}`}
          idx={idx}
        />
